 ...
 * - Posts to http://localhost:4000/api/admin/login
 * - On success, saves JWT to localStorage as 'token'
 * - Verifies session via GET http://localhost:4000/api/admin/me
...
export default function AdminLogin({ onSuccess, redirectTo = "/admin" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const meRes = await fetch("http://localhost:4000/api/admin/me", {
      ...
      >
        ...
        >
          ...
          <div className="mt-10 text-white/50 text-xs">
            <p>Server: http://localhost:4000/api/admin/login</p>
            <p>Self-check: GET http://localhost:4000/api/admin/me</p>
